--=====================================================
-- 1337 Nights | Gather Module (Final: stacked logs + auto toggle-off)
--=====================================================
return function(C, R, UI)
    ...
    local STACK_COLS      = 10
    local STACK_V_GAP     = 0.1
    local STACK_H_GAP     = 0.0
    local TETHER_SPREAD   = 0.4
    local TETHER_RING     = 8
    ...
    local function clamp(v, lo, hi) v = tonumber(v) or lo; return math.clamp(v, lo, hi) end
    ...
    local function groundYAt(pos)
        local rc = WS:Raycast(pos + Vector3.new(0, 100, 0), Vector3.new(0, -500, 0))
        return rc and rc.Position.Y or pos.Y
    end
    local function belowMapCFrameWithOffset(i)
        local root = hrp(); if not root then return nil end
        local baseY = groundYAt(root.Position) - carryDepth
        local basePos = Vector3.new(root.Position.X, baseY, root.Position.Z)
        local ring = math.floor((i-1) / TETHER_RING)
        local idx  = (i-1) % TETHER_RING
        local ang  = (idx / TETHER_RING) * math.pi * 2
    ...
    local function matchesResource(name)
        ...
        if resourceType == "Logs"    then return n:find("log") end
        if resourceType == "Stone"   then return n:find("stone") or n:find("rock") end
        if resourceType == "Berries" then return n:find("berry") end
        return false
    end
    ...
    local function setEnabled(on)
        gatherEnabled = on
        if on then
            resourceType = resourceType or "Logs"
            startTether()
            startScanner()
        else
            stopScanner()
    ...
    local function placeLogStackSequential(list)
        ...
        local basePos=root.Position+root.CFrame.LookVector*5
        ...
        for i,m in ipairs(list) do
            ensurePrimary(m); stopDrag(m); setNoCollide(m,true); showLocal(m)
            local col=i%STACK_COLS
            local row=math.floor(i/STACK_COLS)
            local lateral=(col-halfCols)*stepX
            local height=row*stepY
            local pos=basePos+Vector3.new(lateral,0,0)
            local groundY=groundYAt(pos)
            pos=Vector3.new(pos.X,groundY+height+(s.Y/2),pos.Z)
    ...
    local function setItemsVisible()
        ...
        for _,m in ipairs(snap) do
            if m.Name:lower():find("log") then table.insert(logs,m) end
        end
