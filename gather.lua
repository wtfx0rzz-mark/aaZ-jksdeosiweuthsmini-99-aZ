return function(C, R, UI)
    ...
    -- Max targets processed per wave.
    TUNE.MAX_TARGETS_PER_WAVE     = TUNE.MAX_TARGETS_PER_WAVE     or 8
    -- Characters per wave and pacing.
    ...
    -- Trees: pacing, jitter, and impact computation.
    TUNE.TREE_DEBOUNCE_SEC        = TUNE.TREE_DEBOUNCE_SEC        or 0.15
    TUNE.TREE_HIT_STEP_WAIT       = TUNE.TREE_HIT_STEP_WAIT       or 0.01   -- kept for compatibility (not used in parallel path)
    ...
    -- Movement gating: allow chopping while within this radius from the last anchor position.
    TUNE.TREE_ALLOW_MOVE_RADIUS_STUDS = TUNE.TREE_ALLOW_MOVE_RADIUS_STUDS or 5
    -- Parallelism: max concurrent tree chops per wave.
    TUNE.TREE_PARALLEL            = TUNE.TREE_PARALLEL            or 8
    -- =========================
    ...
    local function chopWave(targetModels, swingDelay, hitPartGetter, isTree)
        ...
        -- Trees: parallel up to TUNE.TREE_PARALLEL
        local cap = math.min(#targetModels, TUNE.MAX_TARGETS_PER_WAVE)
        local active = 0
        local maxWorkers = math.max(1, TUNE.TREE_PARALLEL or 8)

        for i = 1, cap do
            local mdl = targetModels[i]
            local t0 = lastTreeHitAt[mdl] or 0
            if (tick() - t0) >= TUNE.TREE_DEBOUNCE_SEC then
                while active >= maxWorkers do task.wait() end
                active = active + 1
                task.spawn(function()
    ...
    local function startSmallTreeAura()
        ...
        task.spawn(function()
            ...
            while running.SmallTree do
                ...
                if not anchorPos then anchorPos = pos end
                local within = (pos - anchorPos).Magnitude <= (TUNE.TREE_ALLOW_MOVE_RADIUS_STUDS or 5)
                if not within then
                    anchorPos = pos
                    task.wait(0.1)
                else
