...
local PULL_ON = false
local ATTR = "__PulledPI"
local PULL_RADIUS = 15
local BATCH = 10
local QUEUE, MARKED = {}, setmetatable({}, {__mode="k"})
...
local NAME_SET = {}
for _,arr in ipairs({junkItems,fuelItems,foodItems,medicalItems,weaponsArmor,ammoMisc,pelts}) do
    for _,n in ipairs(arr) do NAME_SET[n] = true end
end

local function hasHumanoid(m) return m and m:IsA("Model") and m:FindFirstChildOfClass("Humanoid") ~= nil end
local function isMossyName(n) if n=="Mossy Coin" then return true end return n and n:match("^Mossy Coin%d+$") ~= nil end
local function isKeyLike(nl)
    if not nl then return false end
    if not nl:find(" key",1,true) then return false end
    return nl:find("blue key",1,true) or nl:find("yellow key",1,true) or nl:find("red key",1,true)
        or nl:find("gray key",1,true) or nl:find("grey key",1,true) or nl:find("frog key",1,true)
end
local function isGatherItem(m)
    if not (m and m:IsA("Model")) then return false end
    local name = m.Name or ""
    local nl = name:lower()
    if NAME_SET[name] then return true end
    if isMossyName(name) then return true end
    if nl:find("blueprint",1,true) then return true end
    if name == "Forest Gem" or nl:find("forest gem fragment",1,true) then return true end
    if isKeyLike(nl) then return true end
    ...
    if isSapling(m) then return true end
    return false
end
...
local function collectCandidates()
    ...
    for _,d in ipairs(items:GetDescendants()) do
        local m = d:IsA("Model") and d or d:IsA("BasePart") and d:FindFirstAncestorOfClass("Model") or nil
        if m and m.Parent and isGatherItem(m) and not MARKED[m] and not m:GetAttribute(ATTR) then
            QUEUE[#QUEUE+1] = m
        end
    end
end
...
local function randInCircle(r)
    ...
    local u = math.random()
    local rr = r*math.sqrt(u)
    return rr*math.cos(t), rr*math.sin(t)
end

local function pullOnceBatch()
    local root = hrp(); if not root then return true end
    if #QUEUE == 0 then return true end
    local center = root.Position + Vector3.new(0, 2, 0)
    local processed = 0
    while processed < BATCH and #QUEUE > 0 do
        local m = table.remove(QUEUE)
        if m and m.Parent then
            local mp = mainPart(m)
            if mp then
                local dx, dz = randInCircle(PULL_RADIUS)
                local g = groundBelow(center + Vector3.new(dx, 0, dz))
                local dest = CFrame.new(Vector3.new(g.X, g.Y + 1.5, g.Z), center)
                local snap = snapshotCollision(m)
                setCollisionOff(m)
                if RF_Start then pcall(function() RF_Start:FireServer(m) end) end
                Run.Heartbeat:Wait()
                ...
                if RF_Stop then pcall(function() RF_Stop:FireServer(m) end) end
                setPhysicsRestore(m)
                pcall(function() m:SetAttribute(ATTR, true) end)
                MARKED[m] = true
            end
        end
        processed += 1
...
local function enablePull()
    ...
    pullConn = Run.Heartbeat:Connect(function()
        local done = pullOnceBatch()
        if done then
            if pullConn then pullConn:Disconnect() pullConn = nil end
        end
    end)
...
local function disablePull()
    PULL_ON = false
    if pullConn then pullConn:Disconnect() pullConn = nil end
    for m,_ in pairs(MARKED) do
        if m and m.Parent then pcall(function() m:SetAttribute(ATTR, nil) end) end
    end
    table.clear(MARKED)
    table.clear(QUEUE)
end
...
if tab.Toggle then
    ...
    tab:Button({
        ...
        Callback = function(btn)
            state = not state
            if state then enablePull() else disablePull() end
            if btn and btn.SetTitle then btn:SetTitle("Pull Items: " .. (state and "ON" or "OFF")) end
        end
    })
end
