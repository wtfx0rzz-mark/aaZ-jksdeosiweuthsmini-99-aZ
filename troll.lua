...
return function(C, R, UI)
    ...
    local function adopt(model, tgt, seed)
        ...
        ensureUid(uid)
        active[model] = {
            target=tgt, uid=uid, t0=os.clock(), seed=seed, rng=rngFor((seed or 0) + os.clock()),
            ...
            started=false, snap=nil, lastSeen=os.clock(), lastVel=Vector3.zero,
            nudgeUntil=0, nudgeSide=0
        }
        ...
        task.spawn(function()
            ...
            st.started = safeStartDrag(model)
            task.wait(0.05)
    ...
    local function floatStep(model, st, dt)
        ...
        ensureSlot(st); maybeBurst(st); maybeStartNudge(st, dt)
        local t = (os.clock() - (st.t0 or os.clock())) * CFG.Speed
        local jx = math.sin(t * st.w.xz1 + st.phases.xz1) * JIT_XZ1 + math.cos(t * st.w.xz2 + st.phases.xz2) * JIT_XZ2
        local jz = math.cos(t * st.w.xz1 * 0.8 + st.phases.xz1*0.7) * JIT_XZ1 + math.sin(t * st.w.xz2 * 1.3 + st.phases.xz2*0.5) * JIT_XZ2
        local jy = math.sin(t * st.w.y1  + st.phases.y1 ) * JIT_Y1  + math.cos(t * st.w.y2  + st.phases.y2 ) * JIT_Y2
    ...
    local K = {
        Radius = 15.0, NoSelfRadius = 17.0,
        Cooldown = 1.0, ScanInterval = 0.08, MaxPerScan = 12,
        MaxMass = 200, MaxParts = 80, AllowAnchored = false,
    ...
    local function modelStats(model)
        ...
        for _,p in ipairs(parts) do
            ...
            if count > K.MaxParts then break end
        end
    ...
    local function shouldSkipModel(model, targetChar)
        ...
        if mass > K.MaxMass then return true end
        if count > K.MaxParts then return true end
        if anyAnchored and not K.AllowAnchored then return true end
        return false
    end
    local function kickModelAway(model, targetRoot)
        ...
        task.spawn(function()
            ...
            task.delay(K.RestoreDelay, function() if snap then setCollide(model, true, snap) end end)
            task.delay(K.OwnerRelease, function()
                for _,p in ipairs(getParts(model)) do
                    pcall(function() p:SetNetworkOwner(nil) end)
                    pcall(function() if p.SetNetworkOwnershipAuto then p:SetNetworkOwnershipAuto() end end)
                end
        ...
        if charsFolder then
            for _, mdl in ipairs(charsFolder:GetChildren()) do
                if isNPCModel(mdl) then
                    ...
                    if r and r:IsA("BasePart") then out[#out+1] = { root=r, char=mdl } end
                end
            end
        end
        return out
    ...
    local function scanAndKick()
        ...
        for _,t in ipairs(targets2) do
            ...
            local parts = WS:GetPartBoundsInRadius(root.Position, K.Radius, params) or {}
            local seen = {}
            for _,part in ipairs(parts) do
    ...
    local function ensureEdgeButton()
        ...
        if UI and UI.AddEdgeButton then
            shockEdgeHandle = UI:AddEdgeButton({ Title = "Shockwave", Callback = performShockwave })
        elseif UI and UI.EdgeButtons and UI.EdgeButtons.Add then
            ...
            gui.Name = "ShockwaveEdge"; gui.ResetOnSpawn = false; gui.Parent = pg
            local btn = Instance.new("TextButton")
            btn.Name = "ShockBtn"; btn.Size = UDim2.new(0, 120, 0, 36); btn.Position = UDim2.new(1, -130, 0, 80)
            btn.Text = "Shockwave"; btn.BackgroundColor3 = Color3.fromRGB(30,30,30); btn.TextColor3 = Color3.fromRGB(255,255,255)
            btn.Parent = gui; btn.MouseButton1Click:Connect(performShockwave)
            shockFallbackGui = gui; shockEdgeHandle = true
        end
    end
